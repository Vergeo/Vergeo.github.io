document.addEventListener('DOMContentLoaded', () =>{
	...
	let highScoreText = {
		...
		draw : function () {
			...
			ctx.textAlign = this.align;
			ctx.textBaseline = this.baseline;
			ctx.fillText(`High Score : ${sessionStorage.getItem('highScore')}`, x, y);
		}
	}
	...
	function make_enemy () {
		let enemy = new Garbage(Math.floor(Math.random() * 1000 ) + canvas.width, Math.floor(Math.random() * (canvas.height-100))+50, "assets/garbage3.png", Math.floor(Math.random() * 3)+1);
