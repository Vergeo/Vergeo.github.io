document.addEventListener("DOMContentLoaded", function() {
	...
	class Snake {
		...
		move() {
			...
			if (this.checkCollision(newHead)) {
				// console.log(this.checkCollision(newHead));
				gameOverText.draw();
				return;
			}
		...
		checkCollision(head) {
			...
			const botomCollision = head.row === row - 1;

			const wallCollision = leftCollision || topCollision || rightCollision || botomCollision;
	...
	let mainloop = setInterval(function() {
		...
		// console.log(snake.checkCollision(segments.));
		apple.draw();

		if (start) snake.move();
		else startButton.draw();
		
	}, 100)

	// let block = new Block(3, 5);
	// block.drawSquare("Pink");
})